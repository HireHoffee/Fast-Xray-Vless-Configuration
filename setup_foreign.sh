#!/bin/bash
# =============================================================================
# setup_foreign.sh — Настройка иностранного VPS + установка VPN (Xray/VLESS)
# =============================================================================
# Использование:
#   bash setup_foreign.sh \
#     --domain   ваш_домен        (например: lorem.mooo.com) \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --vpn-script) VPN_SCRIPT_URL="$2"; shift 2 ;;
    *) die "Неизвестный аргумент: $1. Используйте --help для справки." ;;
  esac
done

# ─── Проверка обязательных аргументов ────────────────────────────────────────
[[ -z "$DOMAIN"   ]] && die "Укажите --domain"
...
echo ""
echo -e "${BOLD}════════════════════════════════════════════════════${NC}"
echo -e "${BOLD}   Настройка иностранного VPS                       ${NC}"
echo -e "${BOLD}════════════════════════════════════════════════════${NC}"
echo ""
...
echo ""
echo -e "${BOLD}════════════════════════════════════════════════════${NC}"
echo -e "${BOLD}   Установка VPN (Xray) для домена: ${DOMAIN}${NC}"
echo -e "${BOLD}════════════════════════════════════════════════════${NC}"
echo ""
...
if [[ "$DOMAIN_IP" != "$SERVER_IP" ]]; then
  ...
  warn "Убедитесь что домен привязан к серверу. Продолжаю через 10 секунд..."
  sleep 10
fi

# Запуск VPN скрипта и захват вывода
VPN_OUTPUT_FILE=$(mktemp)
info "Запуск VPN скрипта: ${VPN_SCRIPT_URL}"
set +e  # отключаем остановку на ошибке — VPN скрипты часто выходят с ненулевым кодом
bash -c "$(curl -L "${VPN_SCRIPT_URL}")" -- "${DOMAIN}" 2>&1 | tee "$VPN_OUTPUT_FILE"
VPN_EXIT_CODE=${PIPESTATUS[0]}
set -e  # включаем обратно
VPN_OUTPUT=$(cat "$VPN_OUTPUT_FILE")
rm -f "$VPN_OUTPUT_FILE"
if [[ $VPN_EXIT_CODE -ne 0 ]]; then
  warn "VPN скрипт завершился с кодом ${VPN_EXIT_CODE} (это не всегда ошибка)"
else
  log "VPN скрипт выполнен успешно"
fi

# ─── Итоговый вывод ──────────────────────────────────────────────────────────
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}║         НАСТРОЙКА ЗАВЕРШЕНА — СОХРАНИТЕ ЭТО     ║${NC}"
echo -e "${BOLD}╚══════════════════════════════════════════════════╝${NC}"
echo ""
echo "Remote Server Settings:"
echo "IP              - ${SERVER_IP}"
...
echo "───────────────────────────────────────────"
echo ""
warn "Не забудьте сохранить конфиг vless XHTTP reality EXTRA — он понадобится для настройки RU моста!"
echo ""
